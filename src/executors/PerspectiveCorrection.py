...
PIPELINES = [
    ...
    # agresif pipeline
    {"name":"clahe_unsharp","pre":"clahe","pre_args":{},"thresh":"adaptive","thresh_args":{"block":11,"C":2},"morph":"close_open","morph_args":{"ksize":(5,5),"iterations":2}},
    {"name":"gamma_bilateral_unsharp","pre":"gamma","pre_args":{"gamma":1.8},"thresh":"canny","thresh_args":{"th1":30,"th2":120},"morph":"close_open","morph_args":{"ksize":(5,5),"iterations":2}},
]
...
def _score_quad(quad:np.ndarray,img_shape:Tuple[int,int]) -> float:
    ...
    for i in range(4):
        ...
        v1,v2=p1-p2,p3-p2
        angle=np.degrees(np.arccos(np.clip(np.dot(v1,v2)/(np.linalg.norm(v1)*np.linalg.norm(v2)+1e-6),-1.0,1.0)))
        angles.append(angle)
...
class PerspectiveCorrection(Component):
    ...
    def _find_best_quad(self,img:np.ndarray)->Tuple[np.ndarray,str,float]:
        ...
        for pipe in PIPELINES:
            try:
                proc = _preprocess(img,pipe["pre"],**pipe["pre_args"])
