...
# YENİ EKLENDİ: Renkli arka planları tespit etmek için akıllı pipeline
def _create_background_mask(image_bgr: np.ndarray) -> np.ndarray:
    """Doygun (renkli) arka plan alanlarını HSV uzayında tespit eder."""
...
# YENİ EKLENDİ: Akıllı pipeline'ın ana fonksiyonu
def _pre_color_robust(image_bgr: np.ndarray) -> np.ndarray:
    ...
    l_enhanced[background_mask == 255] = 255  # Arka planı Canny için görünmez yap
...
def _generate_edge_maps(image_bgr: np.ndarray) -> List[np.ndarray]:
    ...
    # Mevcut pipeline'ları ve yeni eklenen akıllı pipeline'ı birleştir
    return [
        _pre_soft(gray),
        _pre_medium(gray),
        _pre_hard(gray),
        _pre_color_robust(image_bgr)  # Akıllı pipeline eklendi
    ]


# ---------------------- Candidate Generation ---------------------- #
# Bu bölümdeki fonksiyonlar orijinal yapısıyla korunmuştur.
def _find_quads_from_contours(binary_img: np.ndarray,
    ...
    img_area = H * W
    min_area, max_area = img_area * min_area_ratio, img_area * max_area_ratio
    candidates = []
...
def _find_quads_from_hough(edges: np.ndarray,
    ...
    if len(approx) > 4:
        ...
        for comb in combinations(range(len(approx)), 4):
            ...
            if area > best_area: best_area = area; best = quad
        if best is None: return []; quad = best.astype(np.float32)
    else:
...
def _score_quad(quad: np.ndarray, image: np.ndarray, edges_for_density: np.ndarray) -> float:
    return (0.22 * _area_score(quad, image) + 0.16 * _convexity_score(quad) +
            0.18 * _angle_score(quad) + 0.12 * _shape_score(quad) +
            0.12 * _aspect_ratio_score(quad) + 0.10 * _edge_density_score(quad, edges_for_density) +
...
def _select_best_quad(image: np.ndarray,
    ...
    for edges, candidates in zip(edge_maps, contour_candidates_per_map):
        for q in candidates: scored.append((q, _score_quad(q, image, edges)))
    if edge_maps:
        mid_edges = edge_maps[len(edge_maps) // 2]
        for q in hough_candidates: scored.append((q, _score_quad(q, image, mid_edges)))
    # ... en iyisini seç
...
class PerspectiveCorrection(Component):
    ...
    def run(self):
        ...
        # 2) Çoklu edge map üret (GÜNCELLENMİŞ FONKSİYON ÇAĞRISI)
        ...
        # GÜNCELLENDİ: Metadata'ya yeni pipeline eklendi
        self.context["method"] = {
            "pipelines": ["soft_adaptive", "medium_clahe_canny", "hard_unsharp_canny", "color_robust_masking"],
