...
def correct_perspective_advanced(image, params=None):
    ...
    def preprocess_variants(img):
        ...
        # Aggressive pipeline: CLAHE + Unsharp + Canny
        v2 = unsharp_mask(clahe(img))
        edges2 = cv2.Canny(v2, 50, 150)
        variants.append(("aggressive", edges2))

        # Soft pipeline: Gaussian blur + adaptive threshold
        v3 = cv2.GaussianBlur(img, (5, 5), 0)
        thr = cv2.adaptiveThreshold(
            v3, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2
        )
        variants.append(("soft", thr))
    ...
    # Eğer hiçbir uygun aday bulunmazsa, Otsu eşikleme ile deneyelim
    if not candidates:
    ...
    if not candidates:
        # Hiç aday yoksa orijinal resmi döndür
        return image
