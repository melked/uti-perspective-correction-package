...
def correct_perspective(img, params: Params):
    ...
    # 🆕 Ek strateji: Otsu threshold
    _, otsu = cv2.threshold(pre, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    otsu = cv2.morphologyEx(otsu, cv2.MORPH_CLOSE, np.ones((5,5), np.uint8))

    # İlk corners denemesi
    corners = detect_corners(edges, params)
    if corners is None:
        corners = detect_corners(otsu, params)

    if corners is None:
        raise ValueError("Belge köşeleri bulunamadı.")

    warped = four_point_transform(img, corners)
    return PILImage.fromarray(warped), corners
...
class PerspectiveCorrection(Component):
    ...
    def bootstrap(config: dict) -> dict:
        default_config = {"resize_longest_edge": 1200}
    ...
    def run(self):
        try:
            ...
            result_img, corners = correct_perspective(img_np, self.params)
            img.value = np.array(result_img)
            self.image = Image.set_frame(img=img, package_uID=self.uID, redis_db=self.redis_db)

            self.context["src_quad"] = corners.tolist()
            self.context["output_size"] = [result_img.size[0], result_img.size[1]]
            self.context["confidence_score"] = 1.0 if corners is not None else 0.0
            self.context["strategy_used"] = "canny+otsu"
            self.context["diagnostics"] = {"input_shape": img_np.shape}

            return build_response(context=self)

        except Exception as e:
            logger.exception("PerspectiveCorrection failed.")
            return build_response(context=self, error=str(e))
