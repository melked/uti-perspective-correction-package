...
# -----------------------------------------------------------------------------
# 1. Geometri, Puanlama ve Yardımcı Fonksiyonlar (Aynı Kalıyor)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# UZMAN STRATEJİLERİ (Aynı Kalıyor)
# -----------------------------------------------------------------------------
...
def stage3_line_reconstructor(image: np.ndarray) -> Optional [np.ndarray]:
    ...
    h_lines.sort(key=lambda x: x [0]); v_lines.sort(key=lambda x: x [0])
    top_line = h_lines [0]; bottom_line = h_lines [-1]
    left_line = v_lines [0]; right_line = v_lines [-1]
    ...
    def run(self):
        ...
        for name, strategy in strategies.items():
            ...
            if candidate_quad is not None:
                print(f"Aday bulundu: '{name}' stratejisi.")
                candidate_quads.append(candidate_quad)

        best_quad = None
        if candidate_quads:
            # Tüm aday konturları birleştirip en iyisini bul
            all_contours = [q.astype(np.int32) for q in candidate_quads]
            best_quad = find_best_quad_from_contours(all_contours, src_img.shape)
            if best_quad is not None:
                warped = _four_point_transform(src_img_orig, best_quad)
            else:
                print("Uyarı: Birden fazla aday bulundu ancak hiçbiri geçerli bir dörtgen değil. Fallback.")
                warped = src_img_orig
                best_quad = np.array([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]], dtype=np.float32)
        else:
            print("Hiçbir strateji belge bulamadı. Fallback olarak tüm görüntü kullanılıyor.")
            warped = src_img_orig
            best_quad = np.array([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]], dtype=np.float32)
