...
# -----------------------------------------------------------------------------
# 1. Geometri ve Gelişmiş Puanlama Fonksiyonları (Değişiklik Yok)
# -----------------------------------------------------------------------------
...
def stage3_content_analyzer(image: np.ndarray) -> Optional[np.ndarray]:
    ...
    for cluster_indices in [[sorted_clusters[0]], [sorted_clusters[0], sorted_clusters[1]]]:
        ...
        # <<< HATA DÜZELTİLDİ: Puanlamaya göndermeden önce maskeden konturlar bulunmalı.
        contours, _ = cv2.findContours(closed, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        quad = find_best_quad_from_contours(contours, image.shape)
