...
class PerspectiveCorrection(Component):
    ...
    def _preprocess_for_detection(self, image: np.ndarray) -> np.ndarray:
        ...
        return cv2.morphologyEx(edged, cv2.MORPH_CLOSE, self.morph_kernel, iterations=2)

    # YENİ METOT
    def _strategy_lab_color_threshold(self, image: np.ndarray) -> np.ndarray:
        lab = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)
        L, A, B = cv2.split(lab)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        cl = clahe.apply(L)
        _, thresh = cv2.threshold(cl, 180, 255, cv2.THRESH_BINARY)
        closed = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, self.morph_kernel, iterations=2)
        return cv2.morphologyEx(closed, cv2.MORPH_OPEN, self.morph_kernel, iterations=1)
    ...
    def _detect_candidates(self, image: np.ndarray) -> List[np.ndarray]:
        # DEĞİŞTİ: Yeni strateji eklendi
        processed_canny = self._preprocess_for_detection(image)
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        blur = cv2.GaussianBlur(gray, (5, 5), 0)
        processed_adaptive = cv2.adaptiveThreshold(blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11,
                                                   2)
        processed_lab = self._strategy_lab_color_threshold(image)

        candidates = []
        candidates.extend(self._find_quad_from_contours(processed_canny, image))
        candidates.extend(self._find_quad_from_contours(processed_adaptive, image))
        candidates.extend(self._find_quad_from_contours(processed_lab, image))
    ...
    def _score_candidate(self, image: np.ndarray, pts: np.ndarray) -> float:
        ...
        v_tr_tl, v_tr_br, v_bl_tl = tr - tl, tr - br, bl - tl
        ...
        width, height = np.linalg.norm(tr - tl), np.linalg.norm(tr - br)
        if min(width, height) == 0: return 0.0
        aspect_ratio = max(width, height) / min(width, height)
        target_ratio = np.sqrt(2)
    ...
    def _select_best_candidate(self, image: np.ndarray, candidates: List[np.ndarray]) -> Optional[np.ndarray]:
        ...
        scored_candidates = sorted([(self._score_candidate(image, c), c) for c in candidates], key=lambda x: x[0],
                                   reverse=True)
        best_score, best_candidate = scored_candidates[0]
        print(f"En iyi adayın puanı: {best_score:.4f}")
        if best_score < self.config["MIN_SCORE_THRESHOLD"]: return None
        return best_candidate

    # YENİ METOT (Hata ayıklama için)
    def _debug_draw_candidates(self, image: np.ndarray, candidates: List[np.ndarray], filename="debug_output.jpg"):
        output_image = image.copy()
        scores = [self._score_candidate(image, c) for c in candidates]
        for i, (score, candidate) in enumerate(zip(scores, candidates)):
            cv2.drawContours(output_image, [candidate.astype(int)], -1, (0, 255, 0), 2)
            text = f"#{i}: {score:.2f}"
            cv2.putText(output_image, text, tuple(candidate[0][0]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        cv2.imwrite(filename, output_image)
        print(f"Debug görüntüsü '{filename}' olarak kaydedildi.")
    ...
    def run(self):
        ...
        # Hata ayıklama için bu satırı geçici olarak aktif edebilirsiniz:
        # if candidates: self._debug_draw_candidates(resized_img, candidates)
